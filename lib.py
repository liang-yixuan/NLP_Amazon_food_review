...
class Review(object):
    def __init__(self, tweetSurfaceForm, category):
        ...
        #when adding the trigram, model will overfit
        #self._trigramList = [(self._bigramList[idx], self._bigramList[idx + 1][-1]) for idx in range(len(self._bigramList) - 1)]
        self._featureSet = set(self._bigramList).union(self.tokenSet)
        
        #adding trigram to feature
        #self._featureSet = set(self._trigramList).union(self._bigramList)
...
def read_csv(path):
    ...
    with open(path) as f:
        ...
        for row in reader:
            if header != 0:
                (Id, ProductId, UserId, ProfileName, HelpfulnessNumerator, HelpfulnessDenominator, Score, Time, Summary, Text) = row
                assert Id not in list(data.keys())
                data[Id] = Review(Text, Score)
            header += 1
    data = list(data.values())  # list of Tweets
    return data


def read_data(path='amazon-fine-food-reviews/reviews_sub.csv'):
    """Returns two lists of tweets: the train set and the test set"""
    data = read_csv(path)
    train_data, test_data = data[:int(TRAIN_SPLIT * len(data))], data[int(TRAIN_SPLIT * len(data)):]
...
def show_tweets(tweets, search_term=None):
    ...
    columns = ['Text', 'Score']
    data = [[str(t), t.category] for t in tweets]
