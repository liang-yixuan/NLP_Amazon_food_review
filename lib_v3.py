...
class Review(object):
    def __init__(self, tweetSurfaceForm, category):
        ...
        self._featureSet = set(self.tokenList)
        ...
        #self._trigramList = [(self._bigramList[idx][0], self._bigramList[idx][1], self._bigramList[idx + 1][-1]) for idx in range(len(self._bigramList) - 1)]
        #self._featureSet = set(self._bigramList).union(self.tokenSet)
        
        #adding trigram to feature
        #self._featureSet = set(self._trigramList).union(self._featureSet)
...
def tok(dataset):
    ...
    for row in dataset:
        ...
        data[Id] = Review(Text, str(Score))
    data = list(data.values())  # list of Tweets
    return data

    
class ReviewBigram(object):
    def __init__(self, tweetSurfaceForm, category):
        if isinstance(tweetSurfaceForm, str):
            self.tokenList = word_tokenize(tweetSurfaceForm)
        else:
...
def tokBigram(dataset):
    data = {}
    for row in dataset:
        (Id, Score, Text) = row
        assert Id not in list(data.keys())
        data[Id] = ReviewBigram(Text, str(Score))
    data = list(data.values())  # list of Tweets
    return data
